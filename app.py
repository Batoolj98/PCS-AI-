...
def biometric_face_gate():
    return render_template('3D Biometric Gate.html')
...
def dashboard():
    if 'user' not in session:
        return redirect(url_for('login'))
    return render_template('Security Operations Center (SOC).html')
...
# ==================== NATIONAL SECURITY PAGES ====================

@app.route('/national-security')
def national_security():
    if 'user' not in session:
        return redirect(url_for('login'))
    return render_template('National Security Operations Center (NSOC).html')

@app.route('/national-cybersecurity')
def national_cybersecurity():
    if 'user' not in session:
        return redirect(url_for('login'))
    return render_template('National Cybersecurity Infrastructure.html')

@app.route('/electronic-warfare')
def electronic_warfare():
    if 'user' not in session:
        return redirect(url_for('login'))
    return render_template('SIGINT & Electronic Warfare Vector.html')

@app.route('/darkweb-intel')
def darkweb_intel():
    if 'user' not in session:
        return redirect(url_for('login'))
    return render_template('Dark Web & Insider Threat Intelligence Center.html')

@app.route('/deepfake-detection')
def deepfake_detection():
    if 'user' not in session:
        return redirect(url_for('login'))
    return render_template('Deepfake Detection Center.html')
...
@app.route('/camera-shield')
def camera_shield():
    if 'user' not in session:
        return redirect(url_for('login'))
    return render_template('AI-Camera-Shield.html')
...
if __name__ == '__main__':
    ...
    print("📍 Dashboard (SOC): http://127.0.0.1:5000/dashboard")
    print("📍 Camera Shield: http://127.0.0.1:5000/camera-shield")
    print("-" * 70)
    print("🛡️ NATIONAL SECURITY PAGES:")
    print("   National Security: /national-security")
    print("   National Cybersecurity: /national-cybersecurity")
    print("   Electronic Warfare: /electronic-warfare")
    print("   Dark Web Intel: /darkweb-intel")
    print("   Deepfake Detection: /deepfake-detection")
    print("-" * 70)
    print("🔐 SECRET PAGES:")
    print("   Shadow Dashboard: /shadow-dashboard?key=phoenix-secret-key-2024")
    print("   Elite Alert: /elite-alert")
    print("   Cyber Deception Vault: /cyber-vault")
    print("-" * 70)
    print("📊 TOTAL PAGES LOADED: 42 Pages")
    print("=" * 70)
